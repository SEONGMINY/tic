...
## ADR-032: day timeline drop 판정은 손가락이 아니라 overlay probe를 기준으로 계산한다
이번 회귀의 마지막 원인은 day scope drop 판정이 `pointerGlobal` 하나만 보고 있었다는 점이다. 긴 블록을 아래로 끌거나, 좌우 edge-hover로 날짜를 넘기는 동안 손가락은 타임라인 경계를 잠깐 벗어날 수 있다. 하지만 실제 블록은 아직 유효 위치에 남아 있을 수 있다. 이 경우 raw pointer 기준 판정은 마지막 프레임에서 `minuteCandidate = nil`을 만들고 `invalidDropRestored`로 끝난다.

결정:

- day timeline의 `minuteCandidate`는 raw finger가 아니라 `timelineDropProbePoint`를 기준으로 계산한다.
- probe는 `overlayFrameGlobal.minY`와 `overlayFrameGlobal.midX`를 drop zone 안으로 clamp한 좌표를 사용한다.
- 같은 날 이동, day edge-hover, scope 전환 직후 drop 모두 같은 규칙을 쓴다.
- timeline layout은 `usable global rect`가 확보되기 전까지 drop 계산에 쓰지 않는다.
- 이 회귀는 순수 geometry 테스트와 실제 simulator UI test 둘 다로 고정한다.
